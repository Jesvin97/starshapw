  ...
  try {
    ...
    if (!appRolesError && appRoles && appRoles.length > 0) {
      ...
      if (appRoleUpdateError) {
        console.error(
          '❌ Error updating app_roles:',
          appRoleUpdateError.message
        );
      } else {
      ...
      appRolesError &&
      !appRolesError.message.includes('does not exist')
    ) {
      console.log(
        '⚠️  Note: app_roles table might not exist or no entries found'
      );
    }
